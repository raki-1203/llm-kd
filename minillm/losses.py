...
class Loss():
    ...
    ) -> TensorType['batch_size', 'response_size']:
        last_rw = 0
        rw_reversed = []
    ...
    def _reg_loss(self, query_ids, response_ids, mask, logits, inf_mask, stats):
        with torch.no_grad():
            t_logits = self.trainer.compute_logits_and_log_probs(query_ids,
                                                                 response_ids,
                                                                 inf_mask,
                                                                 base='teacher',
                                                                 return_logprobs=False)

        xent = get_x_entropy(logits, t_logits, inf_mask, mask)
        s_ent = get_entropy(logits, inf_mask, mask)
    ...
    def ppo_loss(self, batch: PPORLBatch, logits):
        ...
        tokens = torch.cat((query_tensors, response_tensors), dim=1)[:, -self.trainer.max_length:]
        mask = self.trainer.get_mask(tokens)[:, start:end]

        logprobs = get_log_probs(logits, response_tensors, mask, inf_mask)

        advantages = self._get_advantages_and_returns(old_rewards, response_length, mask)
        ...
        with torch.no_grad():
            ...
            cumsum_rewards = all_gather(cumsum_rewards, dim=0, world_size=self.trainer.dp_world_size,
                                        group=self.trainer.dp_gropu).mean(dim=0).item()
            rev_kl = all_gather(rev_kl, dim=0, world_size=self.trainer.dp_world_size,
                                group=self.trainer.dp_group).mean(dim=0).item()
            lens = all_gather(lens, dim=0, world_size=self.trainer.dp_world_size,
                              group=self.trainer.dp_gropu).float().mean(dim=0).item()
            s_lens = all_gather(s_lens, dim=0, world_size=self.trainer.dp_world_size,
                                group=self.trainer.dp_group).float().mean(dim=0).item()
