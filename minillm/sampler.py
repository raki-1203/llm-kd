...
class PPOSampler():
    ...
    Transforms samples from `pipeline` into `PPOBatch` and pushes them into trianer's `store`
    """
    ...
    def run_sample(self, num_rollouts_per_device: int=1024, iter_count: int=0):
        """
        Takes `num_rollouts_per_device` prompts from `pipeline`, samples model and computes the KL againts
        a reference model. It then appends PPOElements to trainer's `store`
        """
        ppo_rl_elements = []
        ...
        while len(ppo_rl_elements) < num_rollouts_per_device:
            try:
                ...
                save_rank(f"Another outer ppo epoch, outer ppo epoch: {self.epochs}",
            ...
            with torch.no_grad():
                mode = 'base'
                gen_out = self.trainer.generate(**batch, return_dict_in_generate=True, mode=mode,
                                                teacher_mixed_sample=(self.args.teacher_mixed_alpha is not None),
                                                output_scores=True)
                ...
                _, rollout_logprobs = self.trainer.compute_logits_and_log_probs(query_ids, response_ids,
                                                                                inf_mask=inf_mask, base=mode)
                # student generation features
                if self.args.teacher_mixed_alpha is not None:
                    s_gen_out = self.trainer.generate(**batch, return_dict_in_generate=True, mode=mode,
                                                      output_scores=True)
                    ...
                    _, s_rollout_logprobs = self.trainer.compute_logits_and_log_probs(query_ids, s_response_ids,
                                                                                      inf_mask=s_inf_mask, base=mode)
                ...
                if self.args.teacher_mixed_alpha is not None:
                    with torch.no_grad():
                        # recompute because of the fp16 loss
                        _, t_rollout_logprobs = self.trainer.compute_logits_and_log_probs(query_ids, response_ids,
                                                                                          inf_mask=inf_mask,
                                                                                          base='teacher')
                # get logprobs and the importance sampling weight w
                with torch.no_grad():
                    if self.args.teacher_mixed_alpha is not None:
                        # raw_logprobs: compute using the new model
                        _, raw_logprobs = self.trainer.compute_logits_and_log_probs(query_ids, response_ids,
                                                                                    inf_mask=inf_mask, base='base')
                        logprobs = raw_logprobs
                        mix_probs = (1 - self.args.teacher_mixed_alpha) * torch.exp(rollout_logprobs.float()) + \
                            self.args.teacher_mixed_alpha * torch.exp(t_rollout_logprobs.float())
            ...
            # Push samples and reward to trainer's rollout storage
            self.trainer.push_to_store(ppo_rl_elements)

            if self.args.save_rollout:
                all_query_ids = all_gather(torch.stack([e.query_tensor
                                                        for e in ppo_rl_elements], dim=0).to(self.trainer.device))
                all_response_ids = all_gather(torch.stack([e.response_tensor
                                                           for e in ppo_rl_elements], dim=0).to(self.trainer.device))
                all_entropy = all_gather(torch.stack([e.entropy
                                                      for e in ppo_rl_elements], dim=0).to(self.trainer.device))
                rollout_save_path = os.path.join(self.args.save, 'rollout_history', str(iter_count))
                if get_rank() == 0:
                    os.makedirs(rollout_save_path, exist_ok=True)
                    torch.save((all_query_ids, all_response_ids, all_entropy),
                               os.path.join(rollout_save_path, 'all.pt'))
