...
class PPOPipeline():
    def __init__(self, args, tokenizer, split, ppo_data_path=None, fix_prompts=False, num=-1):
        ...
        if os.path.exists(os.path.join(ppo_data_path, f'{split}.jsonl')):
            with open(os.path.join(ppo_data_path, f'{split}.jsonl')) as f:
                ...
                self.ppo_answers = [x['output'] if isinstance(x['output'], list) else [x['output']]
                                    for x in self.ppo_raw]
    ...
    def collate(self, samples):
        ...
        for i, (prompt, response) in enumerate(samples):
            ...
            if response is not None:
                ...
                no_model_batch['full_label_ids'][i][len(prompt)-1:len(full_ids)-1] = torch.tensor(response,
                                                                                                  dtype=torch.long)
    ...
    def create_loader(self, batch_size: int, shuffle=True, drop_last: bool = False, num_workers: int = 0) -> DataLoader:
        dp_world_size = dist.get_world_size()
        dp_rank = dist.get_rank()

        sampler = DistributedSampler(self, shuffle=shuffle, drop_last=drop_last, rank=dp_rank,
...
class LMPipeline():
    ...
    def collate(self, samples):
        ...
        for i, samp in enumerate(samples):
            self._process_lm(i, samp, model_batch, no_model_batch)
