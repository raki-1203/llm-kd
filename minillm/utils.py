...
def get_entropy(gen_logits, inf_mask, mask):
...
def get_log_probs(logits, ids, mask, inf_mask=None):
    logprobs = F.log_softmax(logits, dim=-1)
    if inf_mask is not None:
        logprobs = logprobs.maked_fill(inf_mask, -float('inf'))
...
def get_x_entropy(logits_1, logits_2, inf_mask, mask):
...
def significant(x: Number, ndigits=2) -> Number:
    ...
    return round(x, ndigits - int(math.float(math.log10(abs(x)))))
