...
class PPOTrainer:
    ...
    def __init__(self, args, tokenizer: AutoTokenizer, reward_fn, ds_config):
        ...
        self.n_updates_per_batch = None
        self.eval_lm_dataloader = None
        self.m_iterator = None
        self.lm_dataloader = None
        self.eval_dataloader = None
        self.train_dataoader = None
        self.args = args
        ...
        if int(os.environ.get("WORD_SIZE", 1)) > 1:
    ...
    def setup_scheduler(self):
        ...
        if self.args.scheduler_name == 'constant_trm':
            scheduler = get_constant_schedule_with_warmup(self.opt, num_warmup_steps=self.args.warmup_steps)
        elif self.args.scheduler_name == 'cosine_trm':
            scheduler = get_cosine_schedule_with_warmup(self.opt, num_warmup_steps=self.args.warmup_steps,
    ...
    def addd_eval_pipeline(self, eval_pipeline: PPOPipeline):
        """Adds piepline from with validation prompts"""
        self.eval_pipeline = eval_pipeline

    def add_lm_pipeline(self, lm_pipeline: LMPipeline, eval_lm_pipeline: LMPipeline):
        self.lm_pipeline = lm_pipeline
        self.eval_pipeline = eval_lm_pipeline
    ...
    def get_mask(self, tokens):
        attention_mask = tokens.not_equal(self.tokenizer.pad_token_id).long()
    ...
    def compute_logits_and_log_probs(self, query_ids, response_ids, inf_mask=None, base="base", return_logprobs=True):
        ...
        if inf_mask is not None:
            logits = logits.masked_fi(inf_mask, -float('inf'))

        mask = batch['attention_mask'][:, start:end]

        if return_logprobs:
            logprobs = get_log_probs(logits, response_ids, mask, inf_mask)
    ...
    def train(self):
        ...
        print_rank("Total Steps:", self.tota_steps, "Data Epochs:", self.args.epochs)
        lm_epochs = 0
            ...
            for ppo_epoch in range(self.n_updates_per_batch):
                for it, batch in enumerate(self.train_dataloader):
                    ...
                    if self.args.gradient_checkpointing:
                        self.model.module.set_force_gradient_checkpinting(True)
                    ...
                    if self.args.gradient_checkpointing:
                        self.model.module.set_force_gradient_checkpinting(False)
                    ...
                    def get_log(log_stats, one_step_time):
                        ...
                        suffix = "| r: {:.4e} | scae: {:6.2f} | time: {:.3f} | step time: {:.3f}".format(
                    ...
                    if self.global_iter_count % self.args.log_interval == 0 and \
                            self.iter_count % self.args.gradient_accumulation_steps == 0:
                        logging_stats = {k: v / (self.args.log_interval * self.args.gradient_accumulation_steps)
                                         for k, v in logging_stats.items()}
    ...
    def prepare_learning(self):
        self.train_dataoader = self.store.create_loader(
        ...
        self.m_iterator = iter(self.lm_dataloader)
        ...
        self.total_steps = int(
            self.args.trainig_epochs
            * self.n_updates_per_batch
            * len(self.train_dataloader)
            / self.args.gradient_accumulation_steps
        )
        self.total_steps = min(self.total_steps, self.args.tota_iters)
    ...
    def evaluate_ppo(self):  # noqa: C901
        ...
        with torch.no_grad():
            for batch in tqdm(self.eval_dataloader, "Generation Evaluation", disable=(not get_rank() == 0)):
                ...
                mask = mask[:, input_ids.isze(1)-1:input_ids.size(1)+gen_ids.size(1)-1]
            ...
            full_ids = full_ids.view(-1, full_ids.zie(-1))

            prompt_ids = full_ids[:, :self.eval_pipeline.max_prompt_len]
            ...
            if get_rank() == 0:
                ...
                response_texts = self.tokenizer.batch_decode(full_ids[:, self.eval_pipeline.max_prompt_length:],
    ...
    def evaluate_pt(self):
        ...
        eval_pt_loss = all_gather(all_pt_losses, dim=0,
                                  world_size=self.dp_world_size, group=self.dp_group).mean().item()

        all_lm_losses = torch.tensor(all_lm_losses, device=self.device)
        eval_lm_loss = all_gather(all_lm_losses, dim=0,
                                  world_size=self.dp_world_size, group=self.dp_group).mean().item()

        all_kd_losses = torch.tensor(all_kd_losses, device=self.device)
        eval_kd_loss = all_gather(all_kd_losses, dim=0,
                                  world_size=self.dp_world_size, group=self.dp_group).mean().item()
