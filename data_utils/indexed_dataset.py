...
def infer_dataset_impl(path):
    if IndexedDataset.exists(path):
        ...
        print(f'Path should be a basename that both .idx and .bin can be appended to get full filenames.')
        return None
...
class MMapIndexedDataset(torch.utils.data.Dataset):
    class Index(object):
        ...
        def __init__(self, path, skip_warmup=False):
            with open(path, 'rb') as stream:
                magic_text = stream.read(9)
                assert self._HDR_MAGIC == magic_text, (
                    'Index file doesn\'t match expected format. '
                    'Make sure that --dataset-impl is configured properly.'
                )
    ...
    def __getitem__(self, idx):
        if isinstance(idx, int):
            ...
            start, stop, step = idx.indices(len(self))
            if step != 1:
                raise ValueError("Slicing into indexed_dataset must be contiguous")
...
class MMapIndexedDatasetBuilder(object):
    ...
    def merge_file(self, another_file):
        # concatenate index
