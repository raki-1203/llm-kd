...
class LMTrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return self._get_lm(idx)

    def _get_lm(self, idx):
        data = self.lm_ctx[idx]
    ...
    def _process_lm(self, i, samp, model_data, no_model_data, gen_data):
        ...
        no_model_data['loss_mask'][i][:source_len - 1] = 0.0
    ...
    def collate(self, samples):
        ...
        for i, samp in enumerate(samples):
            model_data, no_model_data, gen_data = self._process_lm(i, samp, model_data, no_model_data, gen_data)
