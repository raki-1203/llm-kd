...
def get_args():
    ...
    if args.type == "eval_main":
        ...
        save_path = os.path.join(
            ...
            (
                f"e{args.epochs}-bs{args.batch_size}-lr{args.lr}-G{args.gradient_accumulation_steps}"
                f"-N{args.n_gpu}-NN{args.n_nodes}" +
                (f"-lora-{args.peft_lora_r}-{args.peft_lora_alpha}-{args.peft_lora_dropout}"
                 if args.peft == "lora" else "") +
        ...
        save_path = os.path.join(
            ...
            (
                f"e{args.epochs}-bs{args.batch_size}-lr{args.lr}-G{args.gradient_accumulation_steps}"
                f"-N{args.n_gpu}-NN{args.n_nodes}-kd{args.kd_ratio}" +
                (f"-lora-{args.peft_lora_r}-{args.peft_lora_alpha}-{args.peft_lora_dropout}"
                 if args.peft == "lora" else "") +
                args.save_additional_suffix
            ),
        ...
        save_path = os.path.join(
            ...
            (
                f"{args.ckpt_name}" +
                (f"-{args.peft_name}" if args.peft_name is not None else "") +
                (f"-{args.teacher_ckpt_name}" + f"-{args.teacher_peft_name}" if args.teacher_peft_name is not None else "")
            ),
            (
                f"bs{args.batch_size}-lr{args.lr}-G{args.gradient_accumulation_steps}-N{args.n_gpu}"
                f"-NN{args.n_nodes}-lm{args.lm_coef}-len{args.max_length}" +
