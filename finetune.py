...
def get_optimizer(args, model):
    ...
    if args.peft is not None:
        param_gropus = get_optimizer_params_peft(args, model)
    else:
...
def setup_model_and_optimizer(args, ds_config, device, set_optim=True):
    ...
    if ds_config['fp16']['enabled'] == True:
        import copy
    ...
    model, optimizer, _, lr_scheduler = deepspeed.initialize(
        ...
        mpu=pu if args.model_parallel else None,
...
def prepare_dataset(args, tokenizer):
    ...
    if args.do_train:
        ...
        data['test'] = LMTrainDataset(args, tokenizer, args.data_dir, 'valid', 
                                      args.test_num, args.test_ratio, rng_sample)
    else:
...
def get_distil_loss(args, tokenizer, model, teacher_model, model_batch, no_model_batch, logits):
    ...
    if args.model_parallel:
        distil_losses = mpu.parallel_soft_corss_entropy_loss(logits.float(), teacher_logits.float())
...
def finetune(args, tokenizer: AutoTokenizer, model: deepspeed.DeepSpeedEngine, optimizer: AdamW, lr_scheduler,
    ...
    if args.model_parallel:
        ...
        loss_func = mpu.parallel_cross_entorpy
    else:
    ...
    for epoch in range(args.epochs):
        ...
        for it, (model_batch, no_model_batch, gen_data) in enumerate(train_dataloader):
            ...
            if teacher_model is not None:
                distil_loss = get_distil_loss(args,
                                              tokenizer,
                                              model,
                                              teacher_model,
                                              model_batch,
            ...
            def get_log(log_loss, log_distil_loss, log_time):
                return (
                    ...
                    " | loss: {:.4f} | ds_loss: {:.4f} | lr: {:.4e} | scale: {:10.4f}"
                    " | micro time: {:.3f} | step time: {:.3f}"
                ).format(epoch, step, args.total_iters * args.gradient_accumulation_steps,
                         global_step, args.total_iters, log_loss, log_distil_loss, lr_scheduler.get_last_lr()[0],
            ...
            # Checkpointing
            if (args.save and args.save_interval and global_step % args.save_interval == 0 and
            ...
            # Evaluation
            if (args.eval_interval and global_step % args.eval_interval == 0 and
                    step % args.gradient_accumulation_steps == 0):
...
def evaluate(args, tokenizer, model, dataset: LMTrainDataset, split, epoch, device):
    ...
    with torch.no_grad():
        for it, (model_batch, no_model_batch, gen_data) in enumerate(tqdm(dataloader,
                                                                          desc='Evaluating',
        ...
        if args.eval_gen:
            ...
            with open(os.path.join(eval_dir, 'answers.jsonl'), 'w') as f:
                for resp in responses:
                    f.write(json.dumps({'text': resp}, indent=2, ensure_ascii=False) + '\n')
        else:
            res = {}
